...
## CRW SST @ GAK1 mooring vs WGOA ESR time series
...
## Seward line vs WGOA SST

Jane pulled the Seward line data for 0-50 m August - December. Here I compare her data with the WGOA regional sst.

```{r}
seward<-read.csv("data/sewardline/sewardline_0-50m.csv") %>%
  rename(seward_temp=temp)
...
wgoa_late <- wgoa %>%
  ...
  filter(mon>=8) %>%
  group_by(year) %>%
  summarise(crwsst=mean(meansst))

sst2 <-seward %>%
  full_join(wgoa_late, by="year")

ggplot(data=sst2)+
  geom_line(aes(x=year, y=seward_temp), color="turquoise")+
  geom_line(aes(x=year, y=crwsst), color="maroon")
```

Not nearly as good looking as the purely crw sst data...

```{r}
ggplot()+
  geom_point(data=sst2, aes(x=seward_temp, y=crwsst))+
  stat_smooth(data=sst2, aes(x=seward_temp, y=crwsst), method="lm")

fit4<-lm(crwsst~seward_temp, sst2)
summary(fit4)
```
