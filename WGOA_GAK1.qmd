...
# interesting, a slope lower than 1 (but over half a degree higher intercetp)
```

Run a linear model on annual temp.
...
monthsst<-sst %>%
  mutate(mon=month(read_date2)) %>%#calculate month
  filter(mon %in% c(10,11,12 & year!=2024)) %>% 
  group_by(year) %>%
  summarize(gaksst=mean(gaksst),
            meansst=mean(meansst))
#plot
...
# Here we have a positive slope and the intercept is negative
```
