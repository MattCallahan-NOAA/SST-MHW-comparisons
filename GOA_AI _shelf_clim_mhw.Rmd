...
ggplot()+
 # geom_sf(data=ak_dd%>%st_shift_longitude())+
  ...
  geom_sf(data=lkp, aes(color=depth_strata, fill= depth_strata), size=0.01)+
  geom_sf(data=esr%>%
            filter(Ecosystem_Area%in%c("Gulf of Alaska", "Aleutian Islands")), 
          color="black", fill=NA)+
...
mhwplotfun<-function(x, region){
  ...
  scale_color_manual(values=c("white", "#ffc866"))+
   #  scale_fill_manual(values=c("white", hcol))+
    ggtitle(paste0(region," heatwaves"))+
...
#sorting out data problems
 
ggplot()+
  geom_line(data=mhw_clim, aes(x=t, y=temp-seas))+
  facet_grid(rows=vars(depth_strata), cols=vars(ecosystem_sub))

sst_10_200<-sst_10_200%>% mutate(year=year(READ_DATE))

unique(sst_10_200$year)

#identify missing dates
sst%>%mutate(year=year(read_date))%>%filter(year<2015)%>%summarise(error_start=max(read_date))

sst%>%mutate(year=year(read_date))%>%filter(year>2015)%>%summarise(error_end=min(read_date))
